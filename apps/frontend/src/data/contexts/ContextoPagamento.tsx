...
export function ProvedorPagamento(props: any) {
  ...
  function alterarFormaPagamento(formaPagamento: FormaPagamento) {
    salvarItem('gam3r.formaPagamento', formaPagamento);
    setFormaPagamento(formaPagamento);
  }

  function alterarEntrega(entrega: Partial<PedidoEntrega>) {
    salvarItem('gam3r.entrega', entrega);
    setEntrega(entrega);
  }
      ...
      itens: itens.map(
          (item: ItemCarrinho) =>
              ({
                  ...
                  precoUnitario: item.produto.precoPromo,
              }) as ItemPedido,
      ),
    }
  ...
  useEffect(() => {
    const entrega = obterItem('gam3r.entrega');
    const formaPagamento = obterItem('gam3r.formaPagamento');
